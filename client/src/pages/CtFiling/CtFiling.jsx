// src/pages/CtFiling/CtFiling.jsx
...
export default function CtFiling() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error("Failed to load customers (CT Filing):", err);
  ...
  // 🔵 CT version – go to CT periods route
  const handleGoToPeriods = (customer) => {
    if (!customer?.id) return;
    navigate(`/projects/ct-filing/periods/${customer.id}`);
  };

  return (
    <div className="vat-filing-page">
      {/* ✅ reuse same wrapper class */}
      {/* Header */}
      <div className="page-header">
        <div>
          <h2>CT Filing</h2>
          <p>Select a customer to manage CT filing periods.</p>
        </div>
      </div>

      {/* Customers table only (identical to VAT) */}
