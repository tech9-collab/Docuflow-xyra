...
import {
  startPassportJob,
  pollPassportStatus,
  fetchPassportPreview,
  makePassportDownloadUrl,
} from "../../helper/helper";
...
function PassportImporter() {
  ...
  async function handleConvert() {
    ...
    try {
      // 1) start job
      const { job_id } = await startPassportJob({ files, token });

      // 2) poll until done
      await new Promise((resolve, reject) => {
        const tick = async () => {
          try {
            const s = await pollPassportStatus({ jobId: job_id, token });
            setStatus(s);
            if (s.state === "done") {
      ...
      const preview = await fetchPassportPreview({ jobId: job_id, token });
      const apiDownloadUrl = makePassportDownloadUrl(job_id);

      // 4) go to table page
      navigate("/converts/passport/tableresult", {
        state: {
          title: preview.title || "Passport Results",
          columns: preview.columns || [],
          rows: preview.rows || [],
          downloadLabel: "Download Excel",
          downloadFileName: preview.downloadFileName || "passports.xlsx",
          apiDownloadUrl,
          authToken: token,
        },
      });
      ...
      console.error(e);
      alert(e?.message || "Failed to convert Passport");
    } finally {
      ...
      <div className="imp2-grid">
        <div className="pane-left">
          <div className="upload-card">
            ...
            <div className="upload-sub">Add passport front page (image) or multi-page PDF.</div>
          ...
          {polling && (
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-label="Converting Passports"
            >
