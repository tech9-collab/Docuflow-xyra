...
import { startVisaJob, pollVisaStatus, fetchVisaPreview, makeVisaDownloadUrl } from "../../helper/helper";
...
function VisaImporter() {
  ...
  async function handleConvert() {
    ...
    try {
      const { job_id } = await startVisaJob({ files, token });

      await new Promise((resolve, reject) => {
        const tick = async () => {
          try {
            const s = await pollVisaStatus({ jobId: job_id, token });
            setStatus(s);
      ...
      const preview = await fetchVisaPreview({ jobId: job_id, token });
      const apiDownloadUrl = makeVisaDownloadUrl(job_id);

      navigate("/converts/visa/tableresult", {
        state: {
          title: preview.title || "UAE Visa Results",
          columns: preview.columns || [],
          rows: preview.rows || [],
          downloadLabel: "Download Excel",
          downloadFileName: preview.downloadFileName || "uae_visa.xlsx",
          apiDownloadUrl,
          authToken: token,
        },
      });
      ...
      console.error(e);
      alert(e?.message || "Failed to convert UAE Visa");
    } finally {
      ...
      <div className="imp2-grid">
        <div className="pane-left">
          <div className="upload-card">
            ...
            <div className="upload-sub">Upload visa images or multi-page PDFs (entry permit / residence visa).</div>
          ...
          {polling && (
            <div className="import-overlay" role="dialog" aria-modal="true" aria-label="Converting UAE Visas">
